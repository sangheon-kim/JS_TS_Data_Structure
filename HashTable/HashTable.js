...
var HashTable = /** @class */ (function () {
     ...
     * @memberof HashTable
     */
    HashTable.prototype.hashCode = function (data) {
        var hash = 0, i, chr;
     ...
     * @memberof HashTable
     */
    HashTable.prototype.hash_func = function (key) {
     ...
     * @memberof HashTable
     */
    HashTable.prototype.save_data = function (data, value) {
        var hash_address = this.hash_func(this.hashCode(data)); // 해쉬 키값을 넣어준 후 
     ...
     * @memberof HashTable
     */
    HashTable.prototype.read_data = function (data) {
        var hash_address = this.hash_func(this.hashCode(data)); // 해쉬 값을 얻어낸다.
     ...
     * @memberof HashTable
     */
    HashTable.prototype.chainingSaveData = function (data, value) {
        var index_key = this.hashCode(data); // 해쉬코드 값만 얻어낸다.
        var hash_address = this.hash_func(this.hashCode(data)); // 해쉬코드를 8로 나눈 나머지값을 구한다.
        if (!!this.hash_table[hash_address]) {
            // 키값을 받은 것을 기반으로 hash_address를 확인하여, falsy한 값이 아니라면, 기존에 데이터가 있는 것으로 간주
            var search = false; // default Search는 false로 둔다.
            // this.hash_table[hash_address]가 있는 경우이니, 해당 값들을 순회합니다.
            for (var index = 0; index < this.hash_table[hash_address].length; index++) {
     ...
     * @memberof HashTable
     */
    HashTable.prototype.chainingReadData = function (key) {
        var index_key = this.hashCode(key); // 해쉬코드 값만 얻어낸다.
        var hash_address = this.hash_func(this.hashCode(key)); // 해쉬코드를 8로 나눈 나머지값을 구한다.
        if (!!this.hash_table[hash_address]) {
     ...
     * @memberof HashTable
     */
    HashTable.prototype.linearProbingSave = function (key, value) {
        var index_key = this.hashCode(key); // 해쉬코드 값만 얻어낸다.
        var hash_address = this.hash_func(this.hashCode(key)); // 해쉬코드를 8로 나눈 나머지값을 구한다.
        if (!!this.hash_table[hash_address]) {
            // hash_table에서 해쉬 주소값으로 조회 후 있는 경우
            // 시작 지점을 hash_address를 기준으로 잡고 hash_table의 길이만큼 순회합니다.
            for (var index = hash_address; index < this.hash_table.length; index++) {
    ...
    HashTable.prototype.linearProbingRead = function (key) {
        var index_key = this.hashCode(key); // 해쉬코드 값만 얻어낸다.
        var hash_address = this.hash_func(this.hashCode(key)); // 해쉬코드를 8로 나눈 나머지값을 구한다.
        if (!!this.hash_table[hash_address]) {
            // hash_table에서 해쉬 주소값으로 조회 후 있는 경우
 ...
 *
 * 충돌이 발생하지 않은 경우 - O(1)
 * 충돌이 발생한 경우 - O(n); (충돌이 발생하게 되면, 읽어들이거나, 작성시에 순회를 다돌아야하기 때문에)
 *
 * 사용 예시
 * 1. 16개의 배열에 데이터를 저장하고, 검색할때 O(n)
 * 2. 16개의 데이터 저장공간을 가진 위의 해시 테이블에 데이터를 저장하고, 검색할 때 O(1)
 *
 */
